...
function SelectionModifier() {
  ...
  const { pendingEvent, availableTimes, declineReason } = useSelector(
    (state: RootState) => state.modifySelection
  );
  const { addAvailableTimes, deleteAvailableTimes } = modifySelectionActions;
  ...
  useMemo(() => {
    if (eventModifyId) {
      getPendingEventInfo(eventModifyId, 2);
    }
  }, [eventModifyId]);
  ...
  return (
    <>
      ...
      <div className={'time-list-selector'}>
        ...
        <BottomButton text={'수정 완료'} onClick={handlePutClick} />
      </div>
    </>
  );
}
